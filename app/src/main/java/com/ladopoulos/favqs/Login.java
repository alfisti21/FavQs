...
public class Login extends AppCompatActivity {
    ...
    final String urlLogin = "https://favqs.com/api/session";
                        ...
                        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, urlLogin, jsonPreBody, new Response.Listener<JSONObject>() {
                            ...
                            public void onResponse(JSONObject response) {
                                try {
                                    ...
                                    if(check.matches("21")){
                                        Toast toast = Toast.makeText(getApplicationContext(), "Invalid login or password.", Toast.LENGTH_SHORT);
                                        toast.setGravity(Gravity.CENTER_HORIZONTAL, 0, 0);
                                        toast.show();
                                    }else if(check.matches("22")){
                                        Toast toast = Toast.makeText(getApplicationContext(), "Login is not active.\nContact support@favqs.com.", Toast.LENGTH_SHORT);
                                        toast.setGravity(Gravity.CENTER_HORIZONTAL, 0, 0);
                                        toast.show();
                                    }else if(check.matches("23")){
                                        Toast toast = Toast.makeText(getApplicationContext(), "User login or password\nis missing.", Toast.LENGTH_SHORT);
                                        toast.setGravity(Gravity.CENTER_HORIZONTAL, 0, 0);
                                        toast.show();
                                    }else{
                                        Log.e("TOKEN", response.toString());
